document.addEventListener('DOMContentLoaded', () => {
    ...
    const brands = [
        { name: 'zepto', file: 'zepto.svg' },
        { name: 'meesho', file: 'meesho.svg' },
        { name: 'Groww', file: 'groww.svg' },
        { name: 'Khatabook', file: 'khatabook.svg' },
        { name: 'Apollo Clinic', file: 'apollo-clinic.svg' },
        { name: 'Manipal Hospitals', file: 'manipal-hospitals.svg' },
        { name: 'TVS Motors', file: 'tvs-motors.svg' },
    ...
    const populate = (container) => {
        brands.forEach(brand => {
            ...
            div.setAttribute('title', brand.name);
            
            const logoUrl = brand.file;
            
            div.innerHTML = `<img src="${logoUrl}" alt="${brand.name}" class="company-logo" onerror="this.onerror=null; this.parentElement.innerHTML='<span class=\\'company-name\\'>${brand.name}</span>';">`;
