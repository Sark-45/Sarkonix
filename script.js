document.addEventListener('DOMContentLoaded', () => {
    ...
    class DynamicFavicon {
        ...
        checkTheme() {
            ...
            // Prioritize body class if present, otherwise follow preference
            this.isDarkMode = document.body.classList.length > 0 ? bodyHasDarkMode : prefersDark;
        }
        ...
        draw(glowIntensity) {
            ...
            // Draw shadow/glow behind the background
            if (this.isTabActive) {
                this.ctx.shadowBlur = 4 + (glowIntensity * 4);
                this.ctx.shadowColor = `rgba(255, 255, 255, ${0.15 + glowIntensity * 0.2})`;
            }

            // 1. Draw rounded black background
            this.ctx.fillStyle = '#000000';
            this.ctx.beginPath();
            this.ctx.moveTo(r, 0);
            this.ctx.lineTo(s - r, 0);
            this.ctx.quadraticCurveTo(s, 0, s, r);
            this.ctx.lineTo(s, s - r);
            this.ctx.quadraticCurveTo(s, s, s - r, s);
            this.ctx.lineTo(r, s);
            this.ctx.quadraticCurveTo(0, s, 0, s - r);
            this.ctx.lineTo(0, r);
            this.ctx.quadraticCurveTo(0, 0, r, 0);
            this.ctx.closePath();
            this.ctx.fill();

            // Reset shadow for drawing on top
            this.ctx.shadowBlur = 0;

            // 2. Draw the SVG icon (slightly smaller to fit padded in the black box)
            const p = 6; // padding
            const iS = s - (p * 2);
            
            // Create a small clip or just draw it
            // To color it white, we'll draw it and then fill source-atop
            this.ctx.save();
            this.ctx.drawImage(this.img, p, p, iS, iS);
            this.ctx.globalCompositeOperation = 'source-atop';
            this.ctx.fillStyle = '#ffffff';
            this.ctx.fillRect(p, p, iS, iS);
            this.ctx.restore();
        ...
        animate() {
            ...
            this.angle += 0.05;
            const glowIntensity = (Math.sin(this.angle) + 1) / 2;

            // Occasionally re-check theme if body class might have changed
            if (Math.floor(this.angle * 10) % 50 === 0) {
                this.checkTheme();
            }
    ...
    // Initialize after a short delay to ensure assets are ready
    setTimeout(() => {
        new DynamicFavicon();
    }, 1000);
});
